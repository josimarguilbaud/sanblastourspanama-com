...
export async function GET() {
  ...
  const entries = [
    ...islands
      ...
      .map((e) => ({ title: e.data.name, url: `${PREFIX}/islands/${e.id.replace(`${LOCALE}/`, '')}`, type: 'Île' })),
    ...tours
      .filter((e) => e.id.startsWith(`${LOCALE}/`))
      .map((e) => ({ title: e.data.name, url: `${PREFIX}/tours/${e.id.replace(`${LOCALE}/`, '')}`, type: 'Excursion' })),
    ...packages
      .filter((e) => e.id.startsWith(`${LOCALE}/`))
      .map((e) => ({ title: e.data.name, url: `${PREFIX}/tours/packages/${e.id.replace(`${LOCALE}/`, '')}`, type: 'Forfait' })),
    ...guides
      .filter((e) => e.id.startsWith(`${LOCALE}/`))
